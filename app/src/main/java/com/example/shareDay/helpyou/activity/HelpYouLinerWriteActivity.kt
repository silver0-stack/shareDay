...
import com.example.shareDay.helpme.dto.liner2_PostingData
...
class HelpYouLinerWriteActivity : AppCompatActivity(), RadioGroup.OnCheckedChangeListener {
    //firebase
...
//    private lateinit var tamponFragment: HelpMeTamponFragment
    ...
    private fun writeNewPost(
        ...
        if (myLocation.isEmpty()) {
            ...
            val key = database.child("helpyou_liner").push().key
            if (key == null) {
                Log.w(ContentValues.TAG, "Couldn't get push key for posts")
                return
            }
            val newPost = liner2_PostingData(
                myLocation,
                contents,
                userName,
                uid
            )
            database.child("helpyou_liner").child(key).setValue(newPost).addOnSuccessListener {
