...
import com.example.shareDay.helpme.dto.total2_PostingData
...
class HelpYouTotalWriteActivity : AppCompatActivity(), RadioGroup.OnCheckedChangeListener {
    //firebase
...
//    private lateinit var tamponFragment: HelpMeTamponFragment
    ...
    private fun writeNewPost(
        ...
        if (myLocation.isEmpty()) {
            ...
            val key = database.child("helpyou_total").push().key
            if (key == null) {
                Log.w(ContentValues.TAG, "Couldn't get push key for posts")
                return
            }
            val newPost = total2_PostingData(
                myLocation,
                contents,
                userName,
                uid
            )
            database.child("helpyou_total").child(key).setValue(newPost).addOnSuccessListener {
