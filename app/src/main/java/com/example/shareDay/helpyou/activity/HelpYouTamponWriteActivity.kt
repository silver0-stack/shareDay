...
import com.example.shareDay.helpme.dto.tampon2_PostingData
...
class HelpYouTamponWriteActivity : AppCompatActivity(), RadioGroup.OnCheckedChangeListener {
    //firebase
...
//    private lateinit var tamponFragment: HelpMeTamponFragment
    ...
    private fun writeNewPost(
        ...
        if (myLocation.isEmpty()) {
            ...
            val key = database.child("helpyou_tampon").push().key
            if (key == null) {
                Log.w(ContentValues.TAG, "Couldn't get push key for posts")
                return
            }
            val newPost = tampon2_PostingData(
                myLocation,
                contents,
                userName,
                uid
            )
            database.child("helpyou_tampon").child(key).setValue(newPost).addOnSuccessListener {
