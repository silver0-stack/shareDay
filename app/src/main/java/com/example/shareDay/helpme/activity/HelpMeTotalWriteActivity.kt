...
class HelpMeTotalWriteActivity : AppCompatActivity(), RadioGroup.OnCheckedChangeListener {
    //firebase
    ...
    private fun writeNewPost(
        ...
        if (myLocation.isEmpty()) {
            ...
            val key = database.child("helpme_total").push().key
            if (key == null) {
                Log.w(ContentValues.TAG, "Couldn't get push key for posts")
                return
            }
            val newPost = total_PostingData(
                myLocation,
                contents,
                userName,
                uid
