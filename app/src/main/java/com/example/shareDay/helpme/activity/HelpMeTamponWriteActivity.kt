...
import com.example.shareDay.helpme.dto.tampon_PostingData
...
class HelpMeTamponWriteActivity : AppCompatActivity(), RadioGroup.OnCheckedChangeListener {
    //firebase
...
//    private lateinit var tamponFragment: HelpMeTamponFragment
    ...
    private fun writeNewPost(
        ...
        if (myLocation.isEmpty()) {
            ...
            val key = database.child("helpme_tampon").push().key
            if (key == null) {
                Log.w(ContentValues.TAG, "Couldn't get push key for posts")
                return
            }
            val newPost = tampon_PostingData(
                myLocation,
                contents,
                userName,
                uid
